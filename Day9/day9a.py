...
def create_row_of_last_cols(zeroed_sequence):
    res = []
...
def extrapolate_next(sequence):
    ...
    last_cols = create_row_of_last_cols(zeroed_sequence)

    cur_num = 0
    for i in range(len(last_cols) - 1, -1, -1):
        cur_num = last_cols[i] + cur_num
